...
MLflow 연동 (use_mlflow=True 일 때):
    - 파라미터, 에폭별 메트릭, best_val_accuracy 기록
    - 모델 아티팩트 로깅 (mlflow.pytorch)
    - 실행 ID를 output_model/mlflow_run_id.txt 에 저장 (register_model 컴포넌트에서 사용)
    - MLFLOW_TRACKING_URI 환경변수(ConfigMap) 우선, mlflow_tracking_uri 파라미터 보조
"""
...
def train_classifier(
    ...
    use_mlflow: True 이면 MLflow 실험 추적 활성화
    ...
    if use_mlflow:
        ...
        # ConfigMap 환경변수 우선, 파라미터 보조
        tracking_uri = os.environ.get("MLFLOW_TRACKING_URI", "") or mlflow_tracking_uri
        if not tracking_uri:
            print("[train] WARNING: use_mlflow=True 이지만 MLFLOW_TRACKING_URI 가 설정되지 않음. MLflow 비활성화.")
            use_mlflow = False
        else:
            mlflow.set_tracking_uri(tracking_uri)
            mlflow.set_experiment(mlflow_experiment_name)
            mlflow_run = mlflow.start_run()
            print(f"[train] MLflow run 시작: {mlflow_run.info.run_id} (experiment={mlflow_experiment_name})")
    ...
    # -- MLflow: 최종 메트릭 & 모델 아티팩트 로깅 --
    if use_mlflow and mlflow_run:
        import mlflow
        import mlflow.pytorch

        # 최고 모델 weight 로드 후 아티팩트 로깅
        model.load_state_dict(
            torch.load(os.path.join(output_model.path, "best_model.pt"), map_location="cpu")
        )
        mlflow.log_metric("best_val_accuracy", round(best_val_acc, 4))
        mlflow.pytorch.log_model(model, artifact_path="model")

        # run_id 를 파일로 저장 → register_model 컴포넌트가 읽음
        run_id = mlflow_run.info.run_id
        with open(os.path.join(output_model.path, "mlflow_run_id.txt"), "w") as f:
            f.write(run_id)
        print(f"[train] MLflow run_id={run_id} 저장 완료")
